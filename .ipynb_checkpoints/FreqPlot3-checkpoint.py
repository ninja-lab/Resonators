...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 27 11:12:59 2024
...
delta_R=.00025*R    #this is made up for now 
'''
#total strain 
'''
...
if 'matl_str' not in st.session_state:
    st.session_state['matl_str'] = 'Graphene'
matl = matl_dict[st.session_state['matl_str']]

@st.cache_data
def calc_f(Vg, t, g, R, **kwargs):
    '''
    ...
    er : TOTAL radial strain = ΔR / R - MADE UP FOR NOW 
    rho : mass density [kg/m^3]
    **kwargs : the material dictionary 

    Returns
    -------
    Eq 7.3 in: Nanomechanical Resonators: Toward Atomic Scale
...
def calc_f3(Vg, t, g, d, **kwargs):
    ...
    **kwargs : the material dictionary 
    ...
    Returns
    -------
    Eq 5 in: Resonant nano-electromechanical systems from 2D materials
    ...
    rho = kwargs['rho']
    nu= kwargs['v']
    er0 = kwargs['er'] #INITIAL STRAIN -NEEDS WORK 
    Meff = .271*rho*t*pi*(d_m/2)**2 #from page 6 of "freq tuning" 
    term1  = 4.9*Ey*t_m*er0
    term2 = pi*eps_0*d_m**2*Vg**4 /\
        (8*(1-nu**2)*Ey*t_m*er0**2*g_m**4)
    term3 = -pi*eps_0*d_m**2*Vg**2/(3*g_m**3)
    return sqrt((term1 + term2 + term3)/Meff)/(2*pi)
...
def calc_f2(t=10, gamma=.5, d=2, mode=(0,0), **kwargs ):
    ...
    Returns
    -------
    Equation 2 from: High Frequency MoS2 Nanomechanical Resonators
...
def disp_ans(ans):
   try:
       ...
       return '= NaN'    
   if ans2 == 0:
       return f'= {ans1:.2f}'  + r'\,\text{MHz}'
   else: 
       return f'= {ans1:.2f}' +  r'\cdot'+ \
       r'10^{'+ f'{ans2}' + r'}\,\text{MHz}'
st.sidebar.title("Inputs")

# Using 'with' statement to add elements to the sidebar
with st.sidebar:
...
with col1: 
    ...
    st.latex(r'''
    f_{\text{res,FC}} = \frac{1}{2\pi} \sqrt{\frac{2.4^2 E_Y \varepsilon_r}
                                             {\rho_{m} R^2} - \frac{\epsilon_0}
                                             {0.813 \rho_{m} t g^3} V_G^2}
    
        ''')
    
    ans = calc_f(st.session_state.Vg_sld, st.session_state.t_sld,
                 st.session_state.g_sld, st.session_state.d_sld / 2,
                  **matl)
    st.latex(disp_ans(ans))
    st.latex(r'''
    f_{\text{res,FC}} = \frac{1}{2 \pi} \sqrt{\left[ 4.9 E t\varepsilon_{r0} + 
                        \frac{\pi \epsilon_0}{8 (1 - \nu^2)} 
                        \frac{  d^2}{\varepsilon_{r0}^2 E t g^4} V_g^4 - 
                        \frac{\pi \epsilon_0 d^2}{3 g^3} V_g^2 \right] 
                                              / M_{\text{eff}}}
        ''')
    ans = calc_f(st.session_state.Vg_sld, st.session_state.t_sld,
                 st.session_state.g_sld, st.session_state.d_sld,
                  **matl)
    st.latex(disp_ans(ans))
    ...
    Vg  = np.linspace(0,5) #bias voltage
    
    #for the plot of freq vs thickness, with variable Vg, R, g
    vgbias1 = calc_f(Vg, st.session_state.t_sld, 
               st.session_state.g_sld, R, **matl)  
    vgbias2 = calc_f3(Vg, st.session_state.t_sld, 
               st.session_state.g_sld, st.session_state.d_sld, **matl)  
    fig, ax = plt.subplots()
    #ax.set_xscale('log')
    #ax.set_yscale('log')
    #ax.set_xlim(t[0]*.2, t[-1]*10)
    ax.plot(Vg, vgbias1/1e6, label='exp1', color='red')
    ax.plot(Vg, vgbias2/1e6, label='exp2', color='blue')
    #ax.plot(t, f*1.59/1e6, label = r'$1.59 \cdot f_0 $')
    #ax.plot(t, f*2.14/1e6, label = r'$2.14 \cdot f_0 $')
    #major_formatter= EngFormatter(unit='nm', places=0, sep='')
    #ax.xaxis.set_major_formatter(major_formatter)  
...
f(x) = \alpha + (\beta - \alpha) \exp\left(-\gamma \exp\left(\delta \ln(x)\right)\right)
    ...
    memb_plus_plate = calc_f2(t=t2,gamma = st.session_state.gamma_sld,
                 d = st.session_state.d_sld, mode=st.session_state.mode_sct, **matl)
    memb_f = calc_memb_f(t=t2,gamma = st.session_state.gamma_sld,
                     d = st.session_state.d_sld, mode=st.session_state.mode_sct, **matl)
    plate_f = calc_plate_f(t=t2,gamma = st.session_state.gamma_sld,
                      d = st.session_state.d_sld,mode=st.session_state.mode_sct, **matl)
    vg1_f = calc_f(st.session_state.Vg_sld, t2, st.session_state.g_sld,
                st.session_state.d_sld/2, **matl) 
    vg2_f = calc_f3(st.session_state.Vg_sld, t2, st.session_state.g_sld,
                st.session_state.d_sld, **matl) 
    fig2, ax2 = plt.subplots()
    ax2.plot(t2, memb_plus_plate/1e6)
    ax2.plot(t2, memb_f/1e6, linestyle='--', label='memb')
    ax2.plot(t2, plate_f/1e6, linestyle='--', label='plt')
    ax2.plot(t2, vg1_f/1e6, label='bias eq1', color='red')
    ax2.plot(t2, vg2_f/1e6, label='bias eq2', color='blue')
