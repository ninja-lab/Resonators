...
# -*- coding: utf-8 -*-
"""
Created on Thu May  2 10:58:37 2024
...
def calc_f(Vg, t, g, R, Ey, er, rho, **kwargs):
    ...
    **kwargs : TYPE
    ...
    t_m = t*1e-9
    R_m = R*1e-6
    return (1/(2*pi))*sqrt((2.4**2*Ey*er/(rho*R_m**2))-(eps_0/(.813*rho*t_m*g**3))*Vg**2)
...
with col1: 
    ...
    matl = st.selectbox('Material', ('MoS2', 'Graphene'))
    st.write(matl_dict[matl])
    st.latex(r'''
    f_{\text{res,FC}} = \frac{1}{2\pi} \sqrt{\frac{2.4^2 E_Y \varepsilon_r}{\rho_{m} R^2} - \frac{\varepsilon_0}{0.813 \rho_{m} t g^3} V_G^2}
    
        ''')
    
    Vg = st.slider('$V_G \,\, [V]$', min_value=0.0, max_value=2.0, step=.1)  # 👈 this is a widget
    ...
    st.pyplot(fig=fig)
    st.write(calc_f(Vg, .7e-9, g, R, **MoS2) )
    st.write(calc_f(Vg, .7e-9, g, R, **MoS2)/1e6 )
with col2: 

    t_sld_val = st.session_state['t_sld']
    ans = calc_D(t_sld_val, **MoS2)
    ...
    def calc_f2(t=10, gamma=.5, d=2, **kwargs ):
        '''
        ...
        kd = f2(x, *data[mode])
        rho = kwargs['rho']*t_m
        return (kd/(4*pi))*sqrt((16*D)*(kd**2/4+gamma*d_m**2/(4*D))/(rho*d_m**4))  
    def calc_memb_f(t=10, gamma=.5, d=2, **kwargs):

        #d = 2e-6 #diameter in meters
        d_m = d*1e-6 #diameter in meters
        t_m = t*1e-9 #thickness in meters
        D = calc_D(t_m, **MoS2)
        #gamma=.5 #tension in N/m
        x = gamma*(d_m/2)**2/D
        kd = f2(x, *data[mode])
        rho = kwargs['rho']*t_m
        return kd*sqrt(gamma/rho)/(d_m*2*pi)
    def calc_plate_f(t=10, gamma=.5, d=2, **kwargs):
        t_m = t*1e-9 #thickness in meters
        d_m = d*1e-6 #diameter in meters
        D = calc_D(t_m, **MoS2)
        #d = 2e-6 #diameter in meters
        ...
        kd = f2(x, *data[mode])
        rho = kwargs['rho']*t_m
        return kd**2*sqrt(D/rho)/(d_m**2*2*pi)   
    
    def t_sld_callback():
        st.session_state['t_sld'] = t_sld
        
    def d_sld_callback():
        st.session_state['d_sld'] = d_sld
        
    t_sld = st.slider(r'$\text{thickness} \, t \,\, \text{[nm]}$',
                      min_value=1, max_value=70, step=1,
                      key='my_t_sld', on_change=t_sld_callback)  # 👈 this is a widget
    d_sld = st.slider(r'$\text{diameter} \, d \,\, [\mu\text{m}]$',
                      min_value=2, max_value=50, step=1,
                      key='my_d_sld', on_change=d_sld_callback)
    gamma_sld = st.slider(r'$\text{tension} \, \gamma \,\, [\text{N/m}]$',
                      min_value=.1, max_value=5.0, step=.1,
                      key='my_gamma_sld')
    mode = st.selectbox('Mode', data.keys() ) 
    st.latex(r'''
    f_0 = \left( \frac{kd}{4\pi} \right) \sqrt{\frac{16D}{\rho_a d^4} \left[ \left( \frac{kd}{2} \right)^2 + \frac{\gamma d^2}{4D} \right]}
        ''')
     
    ans = calc_f2(t=t_sld, gamma=gamma_sld, d=d_sld, **MoS2)
...
f(x) = \alpha + (\beta - \alpha) \exp\left(-\gamma \exp\left(\delta \ln(x)\right)\right)
    ...
    f3 = calc_f2(t=t2, **MoS2)
    f4 = calc_memb_f(t=t2, **MoS2)
    f5 = calc_plate_f(t=t2, **MoS2)
    f6 = calc_f(Vg, t2, g, R, **MoS2) 
    st.write(Vg)
    ...
    ax2.set_ylim(5, 5e3)
    st.pyplot(fig=fig2)
    st.write(calc_f(0, .5e-9, g, R, **MoS2) )
